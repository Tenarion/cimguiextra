...
local function getRE()
    ...
    local resN = { "comment2_re", "comment_re", "emptyline_re",
        "functypedef_re", "functype_re", "function_re", "functionD_re", "operator_re", "typedef_st_re", "struct_re",
                    ...
                    if linenumdict then
                        ...
                        if not loca then
                            ...
                            if itparent and itparent.locat then
                                loca = itparent.locat
                                print("parent loca", string.format("%q , %q ", itemold, itemfirstline), #itemfirstline,
                                    loca)
                            else
                                loca = 0
...
local function parseFunction(self, stname, itt, namespace, locat)
    ...
    for i, ar in ipairs(argsTa) do
        ...
        argsTa2[i] = { type = typ, name = name, default = defa, reftoptr = reftoptr, ret = retf, signature = sigf, has_cdecl =
        has_cdecl }
    ...
    table.insert(self.funcdefs,
        { stname = stname, funcname = funcname, args = args, signature = signat, cimguiname = cimguiname, call_args =
        caar, ret = ret })
        ...
        if t.cimguiname then
            ...
            if isIMSTR then
                ...
                if doadd and not FP.NOCHAR then
                    ...
                    newcdefs[#newcdefs + 1] = { stname = t.stname, funcname = t.funcname, args = defT2.args, signature =
                    defT2.signature, cimguiname = defT2.cimguiname, ret = defT2.ret }
                end
            end
        else
            print("not cimguiname in"); M.prtable(t)
        end
    end
    ...
    for numcdef, t in ipairs(FP.funcdefs) do
        if t.cimguiname then
            ...
            if isUDT then
                ...
                FP.funcdefs[numcdef] = { stname = t.stname, funcname = t.funcname, args = args, signature = t.signature, cimguiname =
                t.cimguiname, ret = t.ret }
        ...
        if t.cimguiname then
            ...
            if not defT[1].ret and not defT[1].constructor then --if constructor not processed
                if defT[1].funcname:match("~") then
                    ...
                    newcdefs[#newcdefs + 1] = { stname = def.stname, funcname = def.ov_cimguiname, args = def.args, signature =
                    def.signature, cimguiname = def.cimguiname, call_args = def.call_args, ret = def.ret }
                end
            end
        end
    end
...
function M.Parser()
            ...
            if (it.re_name == "vardef_re" or it.re_name == "functype_re") then     -- or it.re_name == "union_re") then
                if not (it.re_name == "vardef_re" and it.item:match "static") then --skip static variables
                    ...
                    if template then
                        ...
                        if doheader then
                            local templatetypedef = self:gentemplatetypedef(ttype, template,
                                self.templates[ttype][template])
    ...
    function par:parse_struct_line(line, outtab, comment)
        ...
        if line:match(functype_re) then
            ...
            for name in rest:gmatch("([^%s,;]+)%s?[,;]") do
                ...
                table.insert(outtab,
                    { type = typen, template_type = template_type, name = namebitfield or name, bitfield = bitfield, comment =
                    comment })
            end
        end
    end
        ...
        local processer = function(it)
            ...
            elseif it.re_name == "struct_re" or it.re_name == "typedef_st_re" or it.re_name == "class_re" then
                ...
                if structname and not self.typenames[structname] then
                    ...
                    outtab.struct_comments[structname] = next(outtab.struct_comments[structname]) and
                    outtab.struct_comments[structname] or nil
        ...
        M.table_do_sorted(self.defsT, function(k, v)
            ...
            if #v > 1 then
                ...
                for i, t in ipairs(v) do
                    ...
                    t.ov_cimguiname = self.getCname_overload(t.stname, t.funcname, t.signature, t.namespace) or
                    k .. typetoStr(post[i])
                    --check ...
                ...
                for i = 1, #v - 1 do
                    for j = i + 1, #v do
                        if v[i].ov_cimguiname == v[j].ov_cimguiname then
                            local t, tj = v[i], v[j]
                            print("Error caused by Bad overloading " .. t.ov_cimguiname .. " of function ", t.funcname,
                                t.signature, "conflicts with ", tj.funcname, tj.signature)
            ...
            if t.cimguiname then
                ...
                if defT.templated then
                    ...
                    for tname, tnamestr in pairs(templates) do
                        ...
                        table.insert(newcdefs,
                            { stname = stname, funcname = t.funcname, args = args, argsc = argscsinpars, signature =
                            signature, cimguiname = cimguiname, call_args = call_args, ret = t.ret })
                    end
                end -- templated
            end     --cimguiname
        end
    ...
    function par:gen_template_typedef_auto(ttype, te, newte)
        ...
        "\ntypedef struct " .. ttype .. "_" .. newte .. " {" .. table.concat(code) .. "} " .. ttype .. "_" ..
        newte .. ";\n"
    end
        ...
        repeat -- continue simulation
            ...
            if not custom and not manual and not def.templated and not FP.get_skipped(def) then
                if def.constructor then
                    ...
                    local empty = def.args:match("^%(%)") --no args
                    table.insert(tab,
                        "CIMGUI_API " .. def.stname .. "* " .. def.ov_cimguiname ..
                        (empty and "(void)" or def.args) .. "\n")
                    table.insert(tab, "{\n")
                    ...
                    if FP.CONSTRUCTORS_GENERATION then
                        table.insert(tab,
                            "CIMGUI_API void " ..
                            def.ov_cimguiname ..
                            "_Construct(" .. def.stname .. "* self" .. (empty and "" or "," .. def.args:sub(2, -2)) ..
                            ")\n")
                        table.insert(tab, "{\n")
        ...
        if t.cimguiname then
            ...
            if not custom and not manual and not def.templated and not FP.get_skipped(def) then
                ...
                if def.constructor then
                    ...
                    table.insert(outtab,
                        "CIMGUI_API " ..
                        def.stname .. "* " .. def.ov_cimguiname .. (empty and "(void)" or def.args) ..
                        ";" .. addcoment .. "\n")
                    if FP.CONSTRUCTORS_GENERATION then
                ...
                elseif def.destructor then
                    ...
                    if def.stname == "" or def.is_static_function then --ImGui namespace or top level
                        table.insert(outtab,
                            "CIMGUI_API " ..
                            def.ret .. " " .. def.ov_cimguiname .. (empty and "(void)" or def.args) ..
                            ";" .. addcoment .. "\n")
                    else
                        table.insert(outtab,
                            "CIMGUI_API " .. def.ret .. " " .. def.ov_cimguiname .. def.args .. ";" .. addcoment .. "\n")
                    end
                end
            end
            if FP.custom_function_post then
