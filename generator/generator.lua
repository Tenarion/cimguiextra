...
package.path = package.path .. ";../../cimgui/generator/?.dll"
...
--local pipe, err = io.open("../extra/imgui_extra.h", "r")
--if not pipe then
...
--print("IMGUIEXTRA_VERSION", implot_version)
...
local function parseImGuiHeader(header, names)
    ...
    parser.getCname = function(stname, funcname, namespace)
        ...
        local pre = (stname == "") and (namespace and (namespace == "ImGui" and "ig" or namespace .. "_") or "ig") or
            stname .. "_"
        return pre .. funcname
    end
...
local modulename = script_args[3]
local headernames = { unpack(script_args, 4) } -- for instance, {"../extra/imgui_extra.h", "repo/imgui_knobs.h"}

-- get filenames that ends in .h
local headerst = "#include"
local headers = {}
for _, headername in ipairs(headernames) do
    if headername:match("%.h$") then
        headerst = headerst .. ' "' .. headername .. '"'
        headers[#headers + 1] = get_filename_without_extension(headername)
    end
end
...
local parser1 = parseImGuiHeader([[headers.h]], headers)
os.remove("headers.h")

parser1:do_parse()

lfs.touch("./output/overloads.txt")
...
lfs.mkdir("./output/" .. modulename)
lfs.mkdir("../" .. modulename)

local files = {
    "overloads.txt",
    "definitions.lua",
    "structs_and_enums.lua",
    "typedefs_dict.lua",
    "definitions.json",
...
copyfile("./output/" .. modulename .. ".h", "../" .. modulename .. "/" .. modulename .. ".h")
copyfile("./output/" .. modulename .. ".cpp", "../" .. modulename .. "/" .. modulename .. ".cpp")
os.remove("./output/" .. modulename .. ".h")
os.remove("./output/" .. modulename .. ".cpp")

for _, file in ipairs(files) do
    copyfile("./output/" .. file, "./output/" .. modulename .. "/" .. file)
