...
private:
    ...
    // A lambda variable, which will be executed when button in notification is pressed
...
public:
    ...
    inline float getFadePercent()
    {
        ...
        {
            return (1.f - (((float)elapsed - (float)NOTIFY_FADE_IN_OUT_TIME - (float)this->dismissTime) / (float)
                NOTIFY_FADE_IN_OUT_TIME)) * NOTIFY_OPACITY;
        }
...
public:
    ...
    ImGuiToast(ImGuiToastType type, int dismissTime, const char* buttonLabel,
               const std::function<void()>& onButtonPress, const char* format, ...) : ImGuiToast(type, dismissTime)
    {
        NOTIFY_FORMAT(this->setContent, format);
...
namespace ImGui
{
    ...
    void InsertNotification(ImGuiToastType type, int dismissTime, const char* title, const char* buttonLabel, const void* onButtonPress, const char* fmt, ...);
    void InsertNotificationV(ImGuiToastType type, int dismissTime, const char* title, const char* buttonLabel, const void* onButtonPress, const char* fmt, va_list args);
